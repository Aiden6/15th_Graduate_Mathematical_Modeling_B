...
XId = [si2;sid2]';
[Individual_fitness,City_connection]=Single_Individual_fitness(XId,XFp,City_Dist,City_coordinate,City_Population);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%作图%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure(1);
clf;hold on;
...
function [Network_total_value,City_connection]=Single_Individual_fitness(XId,XFp,City_coordinate,City_Locaiton,City_Population)
City_num = size(City_Locaiton,1);

%=============================城市GDP权重处理==============================
City_GDP = [6609 46595 9003 30133 13400 43786 7209 2799 478 13890 4856 19530];
for i=1:length(City_GDP)
    GDP_max = max(City_GDP);
    GDP_norm(i) =  City_GDP(i)/GDP_max;
end
GDP_weight = zeros(City_num,City_num);
for ii=1:City_num
    for jj=ii+1:City_num
        GDP_weight(ii,jj) = sqrt(GDP_norm(ii)*GDP_norm(jj));
    end
end
...
for ii =1:City_num
    for jj=ii+1:City_num
        if (City_connection(ii,jj)>=1)
            ...
            Ind = find(SnrReq>Dist);
            ModRate = Capacity_Table(Ind(end));
            Network_total_value = Network_total_value+ sqrt(City_Population(ii)*City_Population(jj))*Xload2(ii,jj)*GDP_weight(ii,jj);
        end
    end
end
